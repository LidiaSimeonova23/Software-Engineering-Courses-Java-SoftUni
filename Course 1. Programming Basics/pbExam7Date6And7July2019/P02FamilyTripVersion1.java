...
public class P02FamilyTripVersion1 {
    public static void main(String[] args) {
        ...
        double valueOfTheAdditionalExpenses = availableBudget * percentageOfAdditionalExpenses / 100;
        ...
        double differenceInMoney = Math.abs(availableBudget - finalSumOfTheTrip);      //намираме разликата между наличния бюджет и необходимата сума
        if (availableBudget >= finalSumOfTheTrip) {       //Ако сумата е достатъчна
            System.out.printf("Ivanovi will be left with %.2f leva after vacation.", differenceInMoney);
        } else {        //else if (availableBudget < finalSumOfTheTrip)    //Ако сумата НЕ е достатъчна
            System.out.printf("%.2f leva needed.", differenceInMoney);
        }
