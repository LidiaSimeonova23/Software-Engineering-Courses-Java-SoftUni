...
public class P07FootballLeagueVersion1 {
    public static void main(String[] args) {
        ...
        for (int fan = 1; fan <= numberOfFans; fan++) {          //за да си построя цикъла, за име на променливата съм използвала fan вместо i
            String sectorOfCurrentFan = scanner.nextLine();            //за всеки фен се чете секторът, в който се намира - текст с възможности "A", "B", "V" или "G".

            if (sectorOfCurrentFan.equals("A")) {
                numberOfFansInSectorA++;     //numberOfFansInSectorA = numberOfFansInSectorA + 1;     //numberOfFansInSectorA += numberOfFansInSectorA;
            } else if (sectorOfCurrentFan.equals("B")) {
                numberOfFansInSectorB++;
            } else if (sectorOfCurrentFan.equals("V")) {
                numberOfFansInSectorV++;
            } else if (sectorOfCurrentFan.equals("G")) {
