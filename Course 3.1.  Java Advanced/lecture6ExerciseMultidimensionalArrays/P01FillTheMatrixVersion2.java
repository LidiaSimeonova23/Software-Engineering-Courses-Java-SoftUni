...
public class P01FillTheMatrixVersion2 {
    public static void main(String[] args) {
        ...
        if (pattern.equals("A")) {    //в случай че шаблонът е "A"
            fillMatrixPatternA(numbersMatrix);       //извикваме си метода, който създадохме и му подаваме аргумент
        } else if (pattern.equals("B")) {    //или в случай че шаблонът е "В"
    ...
    private static void printMatrix(int[][] matrix) {       //създаваме си метод за принтиране на елементите на матрицата  - този път с foreach (вместо for) цикъл

        for (int[] row : matrix) {       //за всеки масив (от цели числа) от матрицата     /за всеки ред от матрицата
            for (int num : row) {        //за всеки елемент от масива        /за всеки елемент от реда
                System.out.print(num + " ");      //на всяка итерация принтираме текущия елемент и интервал
            }
