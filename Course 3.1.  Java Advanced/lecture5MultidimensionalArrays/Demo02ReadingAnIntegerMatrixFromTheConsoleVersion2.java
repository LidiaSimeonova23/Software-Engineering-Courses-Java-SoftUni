...
public class Demo02ReadingAnIntegerMatrixFromTheConsoleVersion2 {
    public static void main(String[] args) {
        ...
        int[] partsArray = Arrays.stream(scanner.nextLine().split(" ")).mapToInt(Integer::parseInt).toArray();
        int rows = partsArray[0];      //брой редове
        int columns = partsArray[1];   //брой колони
        ...
        for (int i = 0; i < rows; i++) {        //for (int i = 0; i < dataMatrix.length; i++)        //for (int i = 0; i <= dataMatrix.length - 1; i++)
            ...
            for (int j = 0; j < columns; j++) {      //for (int j = 0; j < dataMatrix[i].length; j++)        //for (int j = 0; j <= dataMatrix[i].length - 1; j++)
                dataMatrix[i][j] = rowNumbersArray[j];
            }
