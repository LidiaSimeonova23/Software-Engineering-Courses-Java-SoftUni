...
public class P06TheMostPowerfulWordVersion1 {
    public static void main(String[] args) {
        ...
        while (!input.equals("End of words")) {
            ...
            for (int index = 0; index <= currentWord.length() - 1; index++) {         //for (int index = 0; index < currentWord.length(); index++)
                ...
                // sumOfASCIIValuesOfAllSymbolsOfCurrentWord += currentSymbol;      //ясно е, че дори и да не напишем (int),  ще се вземе целочислената, а не символната стойност на текущия символ
            }
            ...
            switch (currentWord.charAt(0)) {
                ...
                case 'U':
                case 'Y':
                    sumOfASCIIValuesOfAllSymbolsOfCurrentWord *= currentWord.length();     //sumOfASCIIValuesOfAllSymbolsOfCurrentWord = sumOfASCIIValuesOfAllSymbolsOfCurrentWord * currentWord.length();
                    break;
                default:
                    sumOfASCIIValuesOfAllSymbolsOfCurrentWord /= currentWord.length();        //sumOfASCIIValuesOfAllSymbolsOfCurrentWord = sumOfASCIIValuesOfAllSymbolsOfCurrentWord / currentWord.length();
