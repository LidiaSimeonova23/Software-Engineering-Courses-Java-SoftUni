...
public class P03MovieDestinationVersion2 {
    public static void main(String[] args) {
        ...
        //Ако дестинацията е Дубай – 30% отстъпка от крайната цена  -> значи, че крайната цена е 70% от цената
        if (destination.equals("Dubai")) {
            totalPriceForAllDays = totalPriceForAllDays * 70 * 0.01;
        }
        ...
        if (totalPriceForAllDays <= availableBudget) {       //Ако бюджетът е достатъчен
            System.out.printf("The budget for the movie is enough! We have %.2f leva left!", availableBudget - totalPriceForAllDays);
        } else {        //else if (totalPriceForAllDays > availableBudget)     //Ако бюджетът НЕ е достатъчен
            System.out.printf("The director needs %.2f leva more!", totalPriceForAllDays - availableBudget);
        }
