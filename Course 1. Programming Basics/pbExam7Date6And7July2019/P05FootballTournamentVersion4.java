...
public class P05FootballTournamentVersion4 {
    public static void main(String[] args) {
        ...
        if (numberOfPlayedMatches == 0) {      //(numberOfPlayedMatches < 1)       //Ако отборът не е изиграл нито един мач
            ...
            for (int currentMatch = 1; currentMatch <= numberOfPlayedMatches; currentMatch++) {
                String resultFromCurrentMatch = scanner.nextLine();    //Резултатът от изиграната среща в един от горепосочените формати – можем да го разглеждаме като текст (вместо като символ) с възможности "W", "D" и "L"

                int pointsFromCurrentMatch = 0;       //създаваме си променлива за брой спечелени точки от текущия мач
                if (resultFromCurrentMatch.equals("W")) {               //при победа броят на спечелените мачове се увеличава с единица и отборът печели 3 точки
                    numberOfWins++;      //numberOfWins = numberOfWins + 1;       //numberOfWins += 1;
                    pointsFromCurrentMatch = 3;
                } else if (resultFromCurrentMatch.equals("D")) {        //при равенство броят на равенствата се увеличава с единица и отборът печели 1 точка
                    numberOfDraws++;
                    pointsFromCurrentMatch = 1;
                } else if (resultFromCurrentMatch.equals("L")) {        //при загуба броят на загубите се увеличава с единица и отборът не печели точки
                    numberOfLosses++;
                }
