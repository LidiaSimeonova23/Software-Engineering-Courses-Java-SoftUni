...
public class P01CountRealNumbersVersion1 {
    public static void main(String[] args) {
        ...
        double[] numbersArray = Arrays.stream(scanner.nextLine().split(" ")).mapToDouble(Double::parseDouble).toArray();   //от първия ред на конзолата си прочитаме масив от дробни числа (избрахме да е масив, защото няма да модифицираме елементите, а просто ще ги съхранявамем масивите са най-бързо действащата структура)
        ...
        TreeMap<Double, Integer> countMap = new TreeMap<>();     //създаваме си асоциативен масив (от 0 item-a) от вид TreeMap, защото искаме да подредим резултатите във възходящ ред (да сортираме) (ключът ще е от клас Double, стойността ще е от клас Integer)
        ...
        for (double number : numbersArray) {      //за всеки елемент от масив numbersArray
            if (!countMap.containsKey(number)) {      //ако асоц. масив countMap не съдържа ключа (прочетеното число, тоест текущия елемент от масива) ->
                countMap.put(number, 1);               //това число да се добави към асоц. масив (числото да е ключ, стойността да е 1, тъй като това число ще се среща за първи път в асоциативния масив)
        ...
        for (Map.Entry<Double, Integer> entry : countMap.entrySet()) {
            System.out.printf("%.0f -> %d%n", entry.getKey(), entry.getValue());    //принтирането е на отделни редове   - принтираме ключа и срещу него стойността (в случая ключът е даден елемент от масива numbersArray, а стойността е колко пъти се съдържа този елемент в масива numbersArray)
        }
