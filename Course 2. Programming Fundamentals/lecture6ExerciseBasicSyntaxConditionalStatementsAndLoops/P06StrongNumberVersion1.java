...
public class P06StrongNumberVersion1 {
    public static void main(String[] args) {
        ...
        //трябва да намерим всяка цифра от числото (най-лесно е чрез while цикъл), след това да намерим нейния факториел (чрез for цикъл) и накрая да намерим сумата от факториелите на всичките цифри на числото, за да можем да я сравним със самото число
        ...
        while (number > 0) {         //докато има число (докато числото е по-голямо от нула)
            ...
            //след като вече знаем коя е текущата последна цифра на числото, трябва да намерим нейния факториел
            int factorialOfCurrentLastDigit = 1;   //създаваме си променлива за факториел с начална стойност 1, защото ще участва в умножение (в случая не може да е 0, защото ще искаме да умножаваме и да намираме произведението, а 0 по което и да е е 0 и задачата ни ще е грешна) - като умножим едно число по 1, получаваме същото число
            for (int currentMultiplier = currentLastDigit; currentMultiplier >= 1; currentMultiplier--) {          // с for-цикъл ще намерим факториела на всяка цифра от числото - умножаваме числото по всяко число преди него, докато стигнем до 1 - напр. 5! = 5 * 4 * 3 * 2 * 1 = 120
                factorialOfCurrentLastDigit *= currentMultiplier;     //factorialOfCurrentLastDigit = factorialOfCurrentLastDigit * currentMultiplier;       //на всяка итерация произведението става все по-голямо
            }

            sumFactorials = sumFactorials + factorialOfCurrentLastDigit;      // sumFactorials += factorialOfCurrentLastDigit;   //намираме сумата от факториелите на всяка от цифрите на даденото число
