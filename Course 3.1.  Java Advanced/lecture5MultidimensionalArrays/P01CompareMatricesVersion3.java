...
public class P01CompareMatricesVersion3 {
    public static void main(String[] args) {
        ...
        boolean areEqual = true;    //създаваме си булева променлива (в началото приемаме, че матриците са еднакви)
        for (int i = 0; i < rowsFirstMatrix; i++) {
            for (int j = 0; j < columnsFirstMatrix; j++) {
                if (firstMatrix[i][j] != secondMatrix[i][j]) {           //ако текущият елемент от първата матрица е различен от текущия елемент от втората матрица
                    areEqual = false;
                    break;        //прекратяваме изпълнението на вътрешния цикъл
                }
            }

            if (!areEqual) {    //прекратяваме изпълнението и на външния цикъл  (за да се прекъсне целият цикъл)
                break;
            }
        }

        if (areEqual) {           //ако матриците са еднакви
            System.out.println("equal");
        } else {       //в протевен случай (ако матриците не са еднакви)
            System.out.println("not equal");
        }
