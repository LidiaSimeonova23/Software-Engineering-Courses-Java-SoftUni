...
public class P04CatFoodVersion1 {
    public static void main(String[] args) {
        ...
        double sumOfAllFoodInGrams = 0;    //създаваме си променлива за колич. храна (в грамове) на всички котки
        ...
        for (int currentCat = 1; currentCat <= numberOfCats; currentCat++) {
            double foodOfCurrentCatInGrams = Double.parseDouble(scanner.nextLine());             //ще четем коя котка по колко грама храна яде за 1 ден - Х грама храна - реално число в интервала [100.00..400.00]

            sumOfAllFoodInGrams = sumOfAllFoodInGrams + foodOfCurrentCatInGrams;          //намираме колко грама храна изяждат всички котки

            //правим проверка, за да определим към коя група принадлежи котката
            if (foodOfCurrentCatInGrams >= 100 && foodOfCurrentCatInGrams < 200) {                           //ако котката изяжда от 100 (вкл.)  до 200 грама, то тя попада в група 1: малки котки.
            ...
            } else if (foodOfCurrentCatInGrams >= 200 && foodOfCurrentCatInGrams < 300) {                    //ако котката изяжда от 200 (вкл.)  до 300 грама, то тя попада в група 2: големи котки.
                bigCatsCount++;      //bigCatsCount = bigCatsCount + 1;
            } else if (foodOfCurrentCatInGrams >= 300 && foodOfCurrentCatInGrams < 400) {                    //ако котката изяжда от 300 (вкл.)  до 400 грама, то тя попада в група 3: огромни котки.
        ...
        double foodInKilograms = sumOfAllFoodInGrams / 1000;     //намираме колко килограма храна изяждат всички котки (като превърнем грамовете в килограми)

        double totalPriceOfFood = foodInKilograms * 12.45;   //намираме цената на храната за всички котки (за 1 ден)
