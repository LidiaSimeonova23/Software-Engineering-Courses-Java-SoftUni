...
public class PMoreExercises02FromLeftToTheRightVersion2 {
    public static void main(String[] args) {
        ...
        for (int currentLine = 1; currentLine <= numberOfLines; currentLine++) {     //за име на променлива ползваме currentLine вместо i
            ...
            String leftElement = numbersAsTextsArray[0];     //създаваме си String променлива за първия елемент от масива (отляво на текущия ред)
            String rightElement = numbersAsTextsArray[1];      //създаваме си String променлива за втория елемент от масива (отдясно на текущия ред)
            ...
            for (int index = 0; index < leftElement.length(); index++) {         //for (int index = 0; index <= leftElement.length() - 1; index++)
                ...
                //за да можем да извършим събирането, текущият символ ни трябва като число/цифра
                if (Character.isDigit(currentSymbol)) {     //ако текущият символ е цифра      //за да изключим случаите, когато текущият символ е '-' (минус)
                    long currentDigit = Character.getNumericValue(currentSymbol);     //ползваме готов метод, чрез който превръщаме символ (който символ е цифра) в число/цифра   - например символът '2'  става цяло число 2
                    sumLeft += currentDigit;       //sumLeft = sumLeft + currentDigit;     //към домоментната сума от цифрите на лявото число прибавяме текущата цифра
                    //и след всички итерации на този първи вложен for цикъл сме намерили сумата от всички цифри на лявото число
                }
            ...
            for (int index = 0; index < rightElement.length(); index++) {        //for (int index = 0; index <= leftElement.length() - 1; index++)
                ...
                if (Character.isDigit(currentSymbol)) {     //ако текущият символ е цифра      //за да изключим случаите, когато текущият символ е '-' (минус)
                    long currentDigit = Character.getNumericValue(currentSymbol);     //ползваме готов метод, чрез който превръщаме символ (който символ е цифра) в число/цифра   - например символът '2'  става цяло число 2
                    sumRight += currentDigit;       //sumRight = sumRight + currentDigit;    //към домоментната сума от цифрите на дясното число прибавяме текущата цифра
                    //и след всички итерации на този втори вложен for цикъл сме намерили сумата от всички цифри на дясното число
                }
