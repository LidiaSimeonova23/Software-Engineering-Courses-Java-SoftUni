...
public class P07MathPotatoVersion2 {
    public static void main(String[] args) {
        ...
        //с for цикъл обхождаме всички елементи от масива и ги добавяме към приоритетната опашка
        for (int index = 0; index <= namesArray.length - 1; index++) {         //for (int index = 0; index < namesArray.length; index++)
            String currentName = namesArray[index];
            kidsPriorityQueue.offer(currentName);
        }

        //можехме да направим същото и с foreach цикъл:
//        for (String element : namesArray) {
//            kidsPriorityQueue.offer(element);
//        }
        ...
        while (kidsPriorityQueue.size() != 1) {
            for (int currentToss = 1; currentToss < numberOfTosses; currentToss++) {      //for (int currentToss = 0; currentToss < steps - 1; currentToss++)
        ...
        String lastKid = kidsPriorityQueue.peek();
        System.out.println("Last is " + lastKid);           //System.out.print("Last is " + lastKid);
        //System.out.println("Last is " + kidsPriorityQueue.peek());
