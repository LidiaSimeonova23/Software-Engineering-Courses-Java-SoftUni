...
public class P05TopIntegersVersion5 {
        ...
        for (int index = 0; index <= numbersArray.length - 1; index++) {     // for (int index = 0; index < numbersArray.length; index++)     //на всяка итерация на цикъла взимаме текущ елемент от масива (от първия ... до последния)
            ...
            if (index != numbersArray.length - 1) {       //ако текущият индекс е различен от последния индекс (тоест, ако текущ елемент е всеки друг елемент с изключение на последния)
                for (int currentNextIndex = index + 1; currentNextIndex <= numbersArray.length - 1; currentNextIndex++) {      //ще обхождаме последователно всеки елемент от масива, който се намира вдясно от текущия елемент (почваме от следващия елемент и стигаме до последния)
                    if (numbersArray[currentNextIndex] > numbersArray[index]) {     //ако следващият елемент в масива е по-голям от текущия
                        break;     //прекъсваме вътрешния for цикъл (защото вече е ясно, че сме намерили поне едно число, което е по-голямо от текущото число, и е безсмислено да проверяваме останалите следващи числа -> текущото число не е топ)
                    ...
                    if (currentNextIndex == numbersArray.length - 1 && numbersArray[index] != numbersArray[numbersArray.length - 1]) {   //ако този следващ индекс е точно последният индекс в масива и ако стойността на текущия елемент е различна от стойността на последния елемент    //индексите са уникални, затова ще ползваме тях за условието на проверката
