...
public class P02WordSynonymsVersion2 {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) {            //for (int i = 1; i <= n; i++)       //създаваме си for цикъл за n на брой итерации (за да четем входните данни с ключове и стойности)
            ...
            wordsMap.putIfAbsent(word, new ArrayList<>());    //ако в асоциативния масив липсва ключът (който ключ е прочетената от конзолата дума) ->
                                      //с new ArrayList<>() създаваме нов обект в паметта  - лист, защото до момента нямаме създаден такъв             дума) на записа/item-a
            ...
            //wordsMap.get(word)  ни връща лист      - а от урока за листове знаем, че в листа добавяме елементи (на последно място в листа) чрез метода .add(element)       //тоест, чрез .add(synonym) добавяме елемент в листа, което в случая е добавяне на стойност за асоц. масив
        }
